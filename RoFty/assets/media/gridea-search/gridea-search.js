...
(function () {
  var UPDATE_TIME = document
    .getElementById("gridea-search-result")
  ...
  function keywordHighlight(searchedContent) {
    ...
    for (var i = 0; i < searchedContent.matches.length; i++) {
      if (searchedContent.matches[i].key === "content") {
        ...
        var beforeKeyword = searchedPostContent.substring(
          indices[0] - 10,
          indices[0]
        ); // 关键字前10字
        var keyword = searchedPostContent.substring(indices[0], indices[1] + 1); // 关键字
        var afterKeyword = searchedPostContent.substring(
          indices[1] + 1,
          indices[1] + 70
        ); // 关键字后70字
        preview =
          beforeKeyword +
          '<span class="searched-keyword">' +
          keyword +
          "</span>" +
          afterKeyword;
      } else {
